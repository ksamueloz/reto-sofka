 ...
 * @author Kevin Moreno.
 */
public class Manned extends Ship implements Objetive{
    
    Conexion conn = new Conexion();
    Connection cn = conn.conectar();
    
    private int capacity; // Capacidad de pasajeros.
    private String orbit; // Distancia km de la orbita.

    public Manned() {
    }

    public Manned(Integer id, String name, double speed, Integer weight, String propulsionSystem, Integer tonThrust, String fuel, String description) {
    ...
    public boolean createShip() {
        ...
        try {
            ...
            String sql2 =  "INSERT INTO manned (capacity, orbit, ship_idship)";
                   sql2 += "VALUES(?, ?, ?);";
            PreparedStatement statement2 = (PreparedStatement) cn.prepareStatement(sql2);
            statement2.setInt(1, getCapacity());
            statement2.setString(2, getOrbit());
            statement2.setInt(3, Integer.parseInt(data[0]));
    ...
    public boolean updateShip() {
        boolean flag = false;
        try {
            String sql =  "UPDATE SOFKA.SHIP SP, SOFKA.MANNED SS ";
                   ...
                   sql += "SP.idtype ='"+ 3 +"', SS.capacity ='"+ getCapacity() +"', SS.orbit ='" + getOrbit() + "' ";
    ...
    public boolean deleteShip(Integer id) {
        ...
        try {
            String sql  = "DELETE FROM SOFKA.manned WHERE ship_idship ='"+ id +"';";
